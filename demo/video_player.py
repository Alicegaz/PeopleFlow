...
class VideoPlayer(QThread):
    ...
    def notify_frame_updates(self, is_playing, img):
        if self._update_frame_signal is not None:
            self._update_frame_signal.emit(is_playing, img)
    ...
    def run(self):
        ...
        while self._is_playing:
            ...
            if self._frame_idx >= LAST_FRAME_IDX:
                self.notify_frame_updates(0, img)
                break
            self.notify_frame_updates(1, img)
